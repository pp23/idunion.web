# Copied and modified from https://gist.githubusercontent.com/thomaspoignant/5b72d579bd5f311904d973652180c705/raw/291fffac5d91da4066cbddc4902934416f4a8453/Makefile

GOCMD=go
GOTEST=$(GOCMD) test
GOVET=$(GOCMD) vet
BINARY_NAME=dtn-idunion-user
VERSION?=0.0.0
SERVICE_PORT?=3000
...
build: generate-api ## Build your project and put the output binary in out/bin/
	mkdir -p out/bin
	GO111MODULE=on $(GOCMD) build -o out/bin/ ./...

build-release: generate-api ## Build your project as standalone binary without symbol table and debug information
	mkdir -p out/bin
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 GO111MODULE=on $(GOCMD) build -ldflags="-w -s" -o out/bin/ ./...
...
## Test:
test: generate-api ## Run the tests of the project
	env
